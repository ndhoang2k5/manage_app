...
import { 
    ...
    DownloadOutlined, StopOutlined, PrinterOutlined, CheckCircleOutlined, SearchOutlined, HistoryOutlined
...
const ProductionPage = () => {
    ...
    // ... (Các hàm Start, Finish, Receive, History giữ nguyên) ...
    ...
    const handleReceiveGoods = async () => { try { const itemsToReceive = orderSizes.filter(s => s.receiving > 0).map(s => ({ id: s.id, size: s.size, quantity: s.receiving })); if (itemsToReceive.length === 0) return message.warning("Chưa nhập số lượng trả hàng!"); await productionApi.receiveGoods(currentOrder.id, { items: itemsToReceive }); message.success("Đã nhập kho!"); setIsReceiveModalOpen(false); fetchData(); } catch (error) { message.error("Lỗi: " + error.response?.data?.detail); } };
    ...
    // --- IN LỆNH (CẬP NHẬT: HIỂN THỊ TỔNG TIỀN) ---
    ...
    const printContent = () => {
        ...
        printWindow.document.write('<style>body { font-family: "Times New Roman"; padding: 20px; } .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 10px; margin-bottom: 20px; } table { width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #000; } th, td { border: 1px solid #000; padding: 8px; text-align: center; } .footer { margin-top: 40px; display: flex; justify-content: space-between; } .money-table td { text-align: right; }</style></head><body>');
    ...
    return (
        <div>
            ...
            {/* MODAL 1: TẠO LỆNH (THÊM INPUT PHÍ) */}
                    ...
                    <Row gutter={24}>
                        <Col span={8}>
                            <Card size="small" title="1. Thông tin Chung" bordered={false} style={{background: '#f9f9f9', marginBottom: 16}}>
                                ...
                                {/* --- KHU VỰC NHẬP PHÍ (MỚI) --- */}
                                <Row gutter={10}>
                                    <Col span={12}>
                                        <Form.Item label="Phí Vận Chuyển" name="shipping_fee" initialValue={0}>
                                            <InputNumber 
                                                style={{width: '100%'}} 
                                    ...
                                    <Col span={12}>
                                        <Form.Item label="Phụ phí khác" name="other_fee" initialValue={0}>
                                            <InputNumber 
                                                style={{width: '100%'}} 
                                                formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                                            />
            ...
            {/* MODAL 2 & 4: NHẬP HÀNG & LỊCH SỬ (Giữ nguyên) */}
