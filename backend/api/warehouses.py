...
def create_warehouse(request: WarehouseCreateRequest, db: Session = Depends(get_db)):
    service = WarehouseService(db)
    try:
...
def transfer_stock(request: TransferCreateRequest, db: Session = Depends(get_db)):
    service = WarehouseService(db)
    try:
